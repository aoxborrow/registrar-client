...
- **Domain forwarding** (`get/setDomainForwarding`): standard `PUT /restful/v2/domains/{domain}/domain_forwarding` (`{forward_url, is_temporary}`; `is_temporary` false=301, true=302) and framed `PUT .../stealth_forwarding` (`{stealth_url, stealth_title}`). Dynadot forwards the **whole domain**, so our generic `DomainForward[]` maps to at most one `@` rule; read from `domain_info.glue_info` (`glue_type` REGISTRAR_FORWARDING / REGISTRAR_STEALTH_FORWARDING, `forward_url`, `forward_type` permanently/temporarily). There is no clear endpoint — clearing restores Dynadot's default nameservers.
