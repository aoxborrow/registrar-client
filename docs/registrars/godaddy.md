...
clear error. **Domain forwarding is no longer reachable**: the v1
`/v1/domains/forwards` routes 404 and the v2 replacement is reseller-only (403 for
standard accounts), so the forwarding features were dropped. **Not run**:
...
| Domain forwarding / URL redirect            | ✗       | **Not reachable via API** (verified live 2026-08-29): v1 `/v1/domains/forwards/{fqdn}` 404s ("no method to handle request"); the v2 replacement `/v2/customers/{customerId}/domains/forwards/{fqdn}` is **reseller/"API-Users"-only** (403 ACCESS_DENIED for standard accounts). Feature dropped. |
