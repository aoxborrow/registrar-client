...
checkAvailability) also verified. Also verified live (2026-08-29):
`updateContacts` (v1 PATCH, reversible non-identity round-trip — a registrant
name/org/email change would trip the ICANN 60-day lock, so only an address change
was exercised); and `setPrivacy(false)`, which surfaced a real limitation —
GoDaddy's **free privacy ("Free DBP")** can't be disabled via the API: `DELETE
/privacy` returns 409, and the `PATCH {exposeWhois:true}` alternative demands a
full WHOIS-exposure consent block the method can't carry, so the client returns a
clear error. **Domain forwarding** works on the **v2 customer-scoped** route
`/v2/customers/{customerId}/domains/forwards/{fqdn}` (the old v1
`/v1/domains/forwards` routes 404); it needs a `customerId` (the customer UUID, or
a numeric shopper ID the client resolves to the UUID) — verified live with a full
set→read→clear round-trip. **Not run**:
`renewDomain` / `transferIn` (v1-only, paid/irreversible, and OTE purchases need a
paid API Reseller account with Good as Gold — a plain developer key returns
`500 ERROR_UNKNOWN`). See `docs/TODOS.md`.

## Authentication

Two auth schemes coexist: the legacy `sso-key {API_KEY}:{API_SECRET}` header (used with v1/v2, noted as deprecating in 2026) and newer scoped Personal Access Tokens (PAT) sent as `Authorization: Bearer <PAT>` (required for v3, supported everywhere). PATs are generated from the developer dashboard, can carry granular scopes (e.g. `domains.domain:read`, `domains.dns:update`, `domains.transfer:execute`), can be set to expire, and can be revoked individually. Auth is account-scoped (acts on the domains owned by the authenticated GoDaddy account); v2 additionally paths requests under `/v2/customers/{customerId}/domains/...` for customer scoping — this is what the domain-forwarding endpoints use, so configure `customerId` (the customer **UUID**, or a numeric shopper ID the client resolves to the UUID via a `GET /v1/shoppers/{id}?includes=customerId` lookup — that lookup is **sso-key only**, so the numeric form needs Key/Secret while the UUID form works with a PAT alone). No IP allowlisting is documented. Rate limits apply per-credential (recent docs cite roughly 600 requests per ~23-minute window; legacy docs cited 60 req/min), returning `429` with a `Retry-After` header; `RateLimit-*` response headers report remaining quota.
...
| Get/set WHOIS privacy                       | ~       | `DELETE /v1/domains/{domain}/privacy` cancels **paid** privacy; **free privacy ("Free DBP") can't be disabled via API** (DELETE → 409; `PATCH exposeWhois:true` needs a consent block) — verified live 2026-08-29.                                                                                                                                                                                                                                                            |
...
| Domain forwarding / URL redirect            | ✓       | **v2 customer-scoped** `/v2/customers/{customerId}/domains/forwards/{fqdn}` (verified live 2026-08-29; the old v1 `/v1/domains/forwards/{fqdn}` 404s). Per-fqdn `PUT`/`DELETE` (204); requires `customerId` (customer UUID, or a numeric shopper ID resolved via an sso-key-only `GET /v1/shoppers/{id}?includes=customerId` lookup). Write enum is `REDIRECT_PERMANENT`/`REDIRECT_TEMPORARY`; reads return `PERMANENT_REDIRECT`/`TEMPORARY_REDIRECT`; `MASKED` is read-only. |
