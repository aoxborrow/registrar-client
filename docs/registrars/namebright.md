...
| Transfer out / get auth-EPP code     | ~       | "Outbound Push" endpoints initiate/force domain pushes to other NameBright accounts; a standard EPP/auth-code retrieval endpoint for transferring away to another registrar was not found in the documented endpoint list               |
...
- **"Push" transfer model** — NameBright's transfer endpoints are framed around intra-NameBright account-to-account "push" (inbound/outbound), not the standard EPP transfer-in/auth-code-out flow other registrars expose; a true cross-registrar transfer-out/auth-code endpoint was not found in the documented list and needs manual verification against the live Help pages.
