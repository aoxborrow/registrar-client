...
I now have a domain at **every** registrar (I registered `NNNNNN.xyz` at NameSilo
...
`NNNNNN.xyz` (currently at NameSilo) is a **6-digit numeric `.xyz`** in the
...
| ------------ | ------------ | --------------------------------- |
| `NNNNNN.xyz` | NameSilo     | register / renew / transfer tests |
...
- **60-day ICANN transfer lock:** a domain can't be transferred for 60 days after
  registration or a prior transfer. `NNNNNN.xyz` was just registered, so it can't
