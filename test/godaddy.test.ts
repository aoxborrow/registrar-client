...
describe('GoDaddy provider', () => {
  ...
  it('resolves a numeric shopper ID to a customer UUID via an sso-key lookup', async () => {
    const uuid = '373a9a36-de96-4fb6-a005-beb658622652';
    const gd = createRegistrar('godaddy', { apiKey: 'k', apiSecret: 's', customerId: '10000000' });
    const calls = stubHttp(gd, req => {
      if (req.path.startsWith('/v1/shoppers/')) return { shopperId: '10000000', customerId: uuid };
    ...
    expect(calls[0].path).toBe('/v1/shoppers/10000000');
  ...
  it('a numeric shopper ID needs sso-key credentials to resolve (PAT alone fails)', async () => {
    const gd = createRegistrar(
      'godaddy',
      { apiToken: 'gd_pat_x', customerId: '10000000' },
